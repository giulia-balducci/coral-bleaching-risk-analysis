...
# Creating the st.cache_resource for the model/preprocessor to reduce loading time and better user experience
@st.cache_resource
def load_model():
...
rf_model, rf_preprocessor = load_model()
    ...
    if points:
        ...
        with col_predict:
            ...
            if st.button('Predict Bleaching Risk', use_container_width=True):
                ...
                input_df = pd.DataFrame([payload])[ALL_COLS]
                X = rf_preprocessor.transform(input_df)
